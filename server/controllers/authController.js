...
export const register = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    if (registerationAttemptsByUser.length >= 5) {
      return next(
        new ErrorHandler(
          "You have exceeded the number of registration attempts. Please contact supprt.",
          400
        )
      );
    }
...
export const resetPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  if (req.body.password !== req.body.confirmPassword) {
    return next(
      new ErrorHandler("Password and confirm password do no match.", 400)
    );
  }
  ...
  if (
    ...
    return next(new ErrorHandler("Password must 8 and 16 characters.", 400));
  }
